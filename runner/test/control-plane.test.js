...
describe('control-plane client (#184)', () => {
  it('sends the runner token, the lease query and JSON bodies in the Worker\'s shapes', async () => {
    ...
    assert.deepEqual(await cp.register({ placement: 'laptop', credentials: [{ name: 'n', engine: 'opencode', kind: 'keyless', fingerprint: 'abcdefabcdef', concurrency: 1, env: null }] }), { runner_id: 'r' });
    assert.equal(await cp.lease({ engine: 'opencode', kind: 'keyless', wait: 20 }), null);
    await cp.postEvents('job 1', []);
    assert.deepEqual(JSON.parse(calls[0].init.body), { placement: 'laptop', credentials: [{ engine: 'opencode', kind: 'keyless', fingerprint: 'abcdefabcdef', concurrency: 1 }] });
