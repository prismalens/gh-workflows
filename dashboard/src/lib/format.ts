...
/**
 * The browser formats it. No locale and no `timeZone` are passed, so both come
 * from the viewer, which is what every established app does. No zone label: it
 * is the reader's own zone, and `Timestamp` puts the exact UTC instant in the
 * element's `title`. Story: #97.
 */
